...
- создать `cfg/cfg.yaml` на основе `cfg/cfg.yaml.template`
...
- запустить `MongoDB`, которая указана в `cfg.yaml` или воспользоваться командой для локального запуска с настройками по умолчанию. Данную команду достаточно выполнить один раз. Она зарегистрирует сервис, которые будет запускать `MongoDB` при перезапуске системы
...
Управление программой осуществляется через браузер, а важные сообщения приходят в `Telegram`, если настройки были внесены в `cfg.yaml` файл. Для начала использования необходимо:
...
- внести его токен в конфиг и запустить программу
- создать чат с ботом - в ответ на любое сообщение он пришлет информацию о `chat_id`
- внести `chat_id` в конфиг и перезапустить программу с полными настройками телеграмма
...
- Заполнить соответствующую секцию в `cfg.yaml` - название счета вводится произвольное и используется для отображения в web-интерфейсе `POptimizer`. При необходимости могут быть внесены несколько счетов для одного токена или счета для разных токенов
- После запуска программы будут созданы соответствующие счета внутри `POptimizer`, если они не созданы ранее, по ним подтянутся актуальные данные об открытых позициях и последствии будут регулярно обновляться
