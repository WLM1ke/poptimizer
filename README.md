...
Чтобы не хранить секреты в открытом виде, рекомендуется:

-сохранить их в хранилище секретных ключей операционной системы

```bash
uv run poptimizer keychain save YOUR_SECRET_KEY YOUR_SECRET_VALUE
```

- в конфигурационным файлам вместо `YOUR_SECRET_VALUE` использовать специальную заглушку `keychain:YOUR_SECRET_KEY`, которая в момент загрузки будет заменять на `YOUR_SECRET_VALUE` из хранилища секретов операционной системы
...
- так же можно зайти в интерфейс хранилища - секреты сохраняются для приложения `poptimizer`
...
При первом запуске буде создан пустой конфигурационный файл, который можно заполнить по мере необходимости на основе шаблона `cfg/cfg.yaml.template`.
...
Так же можно производить ряд операций с помощью бота. Этот функционал опционален, а для его настройки может потребоваться:

- создать `Telegram` [бота](https://core.telegram.org/bots/features#creating-a-new-bot)
- сохранить токен в хранилище секретов (описанный выше) и внести заглушку для него в конфиг
- запустить программу
- создать чат с ботом - в ответ на любое сообщение он пришлет информацию о `chat_id`
- внести `chat_id` в конфиг и перезапустить программу с полными настройками `Telegram`
...
- сохранить токен в хранилище секретов (описанный выше) и внести заглушку для него в конфиг. Название счета вводится произвольное и используется для отображения в web-интерфейсе `POptimizer`. При необходимости могут быть внесены несколько счетов для одного токена или счета для разных токенов
...
- запустите `MongoDB`, которая указана в `cfg.yaml` или воспользоваться командой для локального запуска с настройками по умолчанию. Данную команду достаточно выполнить один раз. Она зарегистрирует сервис, которые будет запускать `MongoDB` при перезапуске системы
...
- перейти по адресу, указанному `cfg.yaml` файле или [http://localhost:5000](http://localhost:5000) по умолчанию
- добавить в настройках хотя бы один брокерский счет или дождитесь автоматического создания для счетов `Tinkoff`, если они были внесены в `cfg.yaml`
...
- следовать рекомендациям в разделе `Optimization`
