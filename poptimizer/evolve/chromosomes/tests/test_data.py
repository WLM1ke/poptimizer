...
def test_init_all_data():
    chromo = data.Data(dict(history_days=40, batch_size=10, forecast_days=270, ticker_on=-1))
    assert len(chromo.data) == 4
...
def test_setup_phenotype():
    chromosome_data = dict(history_days=40, batch_size=10, forecast_days=270, ticker_on=-1)
...
def test_make_child(monkeypatch):
    ...
    parent = data.Data(dict(batch_size=40, history_days=20, forecast_days=300, ticker_on=1))
    base = data.Data(dict(batch_size=30, history_days=50, forecast_days=270, ticker_on=2))
    diff1 = data.Data(dict(batch_size=20, history_days=60, forecast_days=280, ticker_on=1))
    diff2 = data.Data(dict(batch_size=10, history_days=70, forecast_days=260, ticker_on=8))

    child = parent.make_child(base, diff1, diff2)

    assert isinstance(child, data.Data)
    assert len(child.data) == 4
