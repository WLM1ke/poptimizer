...
class SmartLabGatewayError(config.POptimizerError):
    """Ошибки при загрузке данных со https://www.smart-lab.ru."""
...
class SmartLabGateway:
    ...
    async def get(self) -> pd.DataFrame:
        ...
        if FOOTER not in df.index[-1]:
            raise SmartLabGatewayError("Некорректная html-таблица")
        return df.dropna()
