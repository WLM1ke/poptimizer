...
from poptimizer.domain.dl.features import EmbeddingFeat, EmbeddingFeatValue, Features
...
async def _create_feat(ctx: handler.Ctx, pos_n: int, ticker: domain.Ticker, pos_count: int) -> None:
    ...
    feat.embedding[EmbeddingFeat.ticker] = EmbeddingFeatValue(value=pos_n, size=pos_count)
