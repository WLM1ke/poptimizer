...
from poptimizer.data.updaters import connection
...
def need_update(timestamp: Optional[datetime]) -> bool:
    """Нужно ли обновлять данные о диапазоне доступных торговых дат.

    Если последние обновление было раньше публикации данных о последних торгах, то требуется обновление.
    """
    if timestamp is None or timestamp < end_of_trading_day():
        return True
    return False
